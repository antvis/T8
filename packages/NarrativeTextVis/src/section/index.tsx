...
export function Section({ spec, size = 'normal', pluginManager, plugins }: SectionProps) {
  const innerPluginManager = usePluginCreator(pluginManager, plugins);
  const renderCustomSection = () => {
    if (isCustomSection(spec)) {
      const descriptor = innerPluginManager.getBlockDescriptor(spec.customType);
  ...
  return (
    <Container size={size} as="section" className={cx(getPrefixCls('section'), spec.className)} style={spec.styles}>
      ...
      {isStandardSection(spec) &&
        spec.paragraphs.map((p) => <Paragraph key={v4()} spec={p} size={size} pluginManager={innerPluginManager} />)}
    </Container>
  );
}
